...
import net.topikachu.rag.service.chat.strategy.DeepSeekChatModelStrategy;
...
public class ChatService {
        ...
        return retrievalPipeline.retrieveWithParentContexts(userInput, currentUserContext, searchScope, hybridTopK, rerankTopK,
                ...
                .flatMap(retrievalResult -> tracingSupport.traceMono("rag.context_build",
                        ...
                        .flatMap(context -> {
                            ...
                            return callSourcedAnswer(strategy, context, userInput, conversationId)
                                    ...
                                    .map(result -> {
                                        ...
                                        if (strategy instanceof DeepSeekChatModelStrategy) {
                                            saveChatMemory(conversationId, userInput, answer);
                                        }
    ...
    private Mono<SourcedAnswerResult> callSourcedAnswer(ChatModelStrategy strategy,
                                                        String context,
                                                        String userInput,
                                                        String conversationId) {
        if (strategy instanceof DeepSeekChatModelStrategy deepSeekStrategy) {
            return Mono.fromCallable(() -> conversationId == null
                    ...
                    .flatMap(history -> reactiveChatGateway.callSourcedAnswerTool(
                            deepSeekStrategy.getOpenAiApi(),
                            deepSeekStrategy.getModel(),
                            buildSourcedAnswerToolPrompt(),
    ...
    private void saveChatMemory(String conversationId, String userInput, String answer) {
    ...
    static String buildSourcedAnswerToolPrompt() {
        return """
                你是一个专业的“校园智能知识库问答助手”。你必须基于【知识库上下文】回答。
