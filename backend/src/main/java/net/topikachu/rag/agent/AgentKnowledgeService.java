...
public class AgentKnowledgeService {
    ...
    public AgentKnowledgeService(HybridSearchService hybridSearchService,
        ...
        return hybridSearchService.hybridSearch(query, tagsAnyOf, hybridTopK)
                .flatMap(candidates -> {
                    ...
                    return rerankService.rerank(query, candidates, effectiveTopK)
                            .onErrorResume(error -> {
                                log.warn("Rerank failed in tool-backed search, fallback to raw candidates: {}", error.getMessage());
